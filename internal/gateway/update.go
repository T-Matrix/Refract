...
- 白名单域名支持可选的累计单向下行额度，并显示已用、剩余、进度和达限状态。
- 统计报表的后端与客户端排行显示相同周期合计，并明确各自的聚合口径。

## 修复内容

- 后端与客户端流量使用同一字节计数和同一 SQLite 事务，修复异步队列、未定位 IP、保留周期和时间边界造成的误差。
- 升级前无法准确归属到客户端的差额单列为“历史未归属（升级前）”，不再隐藏统计缺口。
- 域名额度保留小数精度，设置值与面板显示一致。

## 安全

- 同一域名的 HTTP、并发流和 WebSocket 共用原子额度，达到上限后停止现有传输并拒绝新请求。
- 额度只累计实际成功写给客户端的单向下行正文，并持久化到 SQLite。`
